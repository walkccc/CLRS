...
For each pair of keys $k$, $l$, where $k \ne l$, define the indicator random variable $X\_{kl} = \text I\\{h(k) = h(l)\\}$. Since we assume simple uniform hashing, $\Pr\\{X\_{kl} = 1\\} = \Pr\\{h(k) = h(l)\\} = 1 / m$, and so $\text E[X\_{kl}] = 1 / m$.

Now define the random variable $Y$ to be the total number of collisions, so that $Y = \sum\_{k \ne l} X\_{kl}$. The expected number of collisions is 
