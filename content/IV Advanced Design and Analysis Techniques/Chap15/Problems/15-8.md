...
Let $S^\*$ be a seam of minimum disruptive measure among all seams that start with pixel $A[i, j]$. Let $A[i + 1, p]$, where $p \in \{j - 1, j, j + 1\}$, be the pixel of $S^\*$ in the next row. Let $S^\prime$ be the sub-seam of $S^\*$ that starts with $A[i + 1, p]$. We claim that $S^\prime$ has the minimum disruptive measure among seams that start with $A[i + 1, p]$. Why? Suppose there exists another seam $S^{\prime\prime}$ that starts with $A[i + 1, p]$ and has disruptive measure less than that of $S^\prime$. By using $S^{\prime\prime}$ as the sub-seam instead of $S^\prime$, we can obtain another seam that starts with $A[i, j]$ and has a disruptive measure which is less than that of $S^\*$. Thus, we obtain a contradiction to our assumption that $S^\*$ is a seam of minimum disruptive measure.
