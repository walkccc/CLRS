...
> Give pseudocode to reconstruct an $\text{LCS}$ from the completed $c$ table and the original sequences $X = \langle x\_1, x\_2, \ldots, x\_m \rangle$ and $Y = \langle y\_1, y\_2, \ldots, y\_n \rangle$ in $O(m + n)$ time, without using the $b$ table.
...
Given a list of numbers $L$, make a copy of $L$ called $L^\prime$ and then sort $L^\prime$.
...
Then, just run the $\text{LCS}$ algorithm on these two lists. The longest common subsequence must be monotone increasing because it is a subsequence of $L^\prime$ which is sorted. It is also the longest monotone increasing subsequence because being a subsequence of $L^\prime$ only adds the restriction that the subsequence must be monotone increasing. Since $|L| = |L^\prime| = n$, and sorting $L$ can be done in $o(n^2)$ time, the final running time will be $O(|L||L^\prime|) = O(n^2)$.
