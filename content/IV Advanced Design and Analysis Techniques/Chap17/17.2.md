...
Let $c\_i =$ csot of $i$th operation.
...
Charge each operation $3$ (amotized cost $\hat c\_i$).
...
Since the amortized cost is $\$3$ per operation, $\sum\limits\_{i = 1}^n \hat c\_i = 3n$. 

We know from Exercise 17.1-3 that $\sum\limits\_{i = 1}^n \hat c\_i < 3n$.
