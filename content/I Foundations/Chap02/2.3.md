...
2. Form the set $S^\prime = \\{z: z = x - y$ for some $y \in S\\}$.
3. Sort the elements in $S^\prime$.
4. Merge the two sorted sets $S$ and $S^\prime$.
...
Suppose that some value $w$ appears twice. Then $w$ appeared once in $S$ and once in $S^\prime$. Because $w$ appeared in $S^\prime$, there exists some $y \in S$ such that $w = x - y$, or $x = w + y$. Since $w \in S$, the elements $w$ and $y$ are in $S$ and sum to $x$.

Conversely, suppose that there are values $w, y \in S$ such that $w + y = x$. Then, since $x - y = w$, the value $w$ appears in $S^\prime$. Thus, $w$ is in both $S$ and $S^\prime$, and so it will appear twice in the merged output.
