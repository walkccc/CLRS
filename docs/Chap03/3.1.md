...
$T(n)$: running time of algorithm **A**. We just care about the upper bound and lower bound of $T(n)$.
The statement: $T(n)$ is at least $O(n^2)$.
Upper bound: Because "$T(n)$ is at least $O(n^2)$", then there's no information about upper bound of $T(n)$.
Lower bound: Assume $f(n) = O(n^2)$, then the statement: $T(n) \ge f(n)$, but $f(n)$ could be any fuction that is "smaller" than $n^2$. For example, constant, $n$, ... So there's no conclusion about lower bound of $T(n)$ too. 
