...
1. If $(x, y) \neq (u, v)$, then $w(x, y) < w(u, v)$ (since $(x, y)$ is the unique light edge). Replacing $(u, v)$ with $(x, y)$ in $T$ gives a spanning tree $T'' = T - \{(u, v)\} \cup \{(x, y)\}$ with total weight less than $T$, contradicting the minimality of $T$.

2. If $(x, y) = (u, v)$, then $(u, v)$ is the unique light edge crossing the cut $(T_u, T_v)$. Since $(u, v) \notin T'$, there must be an alternative path from $u$ to $v$ in $T'$, which must cross the cut $(T_u, T_v)$ via some edge $e \neq (u, v)$. Because $(u, v)$ is the unique light edge, we have $w(u, v) < w(e)$. Replacing $e$ with $(u, v)$ in $T'$ gives a spanning tree $T'' = T' - \{e\} \cup \{(u, v)\}$ with total weight less than $T'$, again contradicting the minimality of $T'$.
