...
To prove the claim, we use a cut-and-paste argument. If $p'$ were not a longest path, then there exists a longer path $p^{\prime\prime}$ from $v$ to $t$. We could cut out $p'$ and paste in $p^{\prime\prime}$ to produce a path $u \to v \overset{p^{\prime\prime}}{\leadsto} t$ which is longer than $p$, thus contradicting the assumption that $p$ is a longest path from $u$ to $t$.
