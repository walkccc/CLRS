...
```cpp
HEAP-DELETE(A, i)
    HEAP-INCREASE-KEY(A, i, A[1] + 1)
    HEAP-EXTRACT-MAX(A)
```

**Note:** The original and alternative solutions might fail in a situation where all keys are equal and the heap elements hold handles to different application objects. The two-liner works as expected in this case, assuming no key is infinity and ignoring possible integer overflow problems, although it is slightly less efficient.

Consider, for example, the following heap of pairs (key, data):
...
Calling HEAP-DELETE(A, 2) should produce 
...
but calling the original algorithm will produce
...
**Note 2:** The following algorithm is wrong. For example, given an array $A = [15, 7, 9, 1, 2, 3, 8]$ which is a max-heap, and if we delete $A[5] = 2$, then it will fail.
