...
3. For Tree Edge and Forward Edge $(u,v)$, we must have $u.d<v.d<v.f<u.f$, then it could be $\text{GRAY}$ to $\text{WHITE}$,  $\text{GRAY}$ to $\text{GRAY}$, $\text{GRAY}$ to $\text{BLACK}$.
4. For Back Edge $(u,v)$, we must have $v.d<u.d<u.f<v.f$, then it could be $\text{WHITE}$ to $\text{GRAY}$, $\text{GRAY}$ to $\text{GRAY}$, $\text{BLACK}$ to $\text{GRAY}$.
...
1. Theorem 22.10: In a depth-first search of an undirected graph G, every edge of G is either a tree
edge or a back edge. So Tree edge and back edge only.
