...
We guess $T(m) \ge cm^2 \forall m < n $,
To Prove $T(n) \ge cn^2$ forall n> $n_0$ 
$$
\begin{aligned}
T(n) & =   \max_{0 \le q \le n - 1} (T(q) + T(n - q - 1)) + \Theta(n) \\\\
     & \ge \max_{0 \le q \le n - 1} (cq^2 + c(n - q - 1)^2) + \Theta(n) \\\\
     & \ge \max_{0 \le q \le n - 1} (cq^2 + c(n - (q + 1))^2) + \Theta(n) \\\\
     & \ge c\max_{0 \le q \le n - 1} (q^2 + n^2 - 2nq - 2n + q^2 + 2q +1) + \Theta(n) \\\\
     & \ge c\max_{0 \le q \le n - 1} (2q^2 + n^2 - 2nq - 2n + 2q +1) + \Theta(n) \\\\
Let f(q) & = 2q^2 + n^2 - 2nq - 2n + 2q +1  \\\\
f'(q) & = 4q -2n + 2 \\\\
f''(q) & = 4 = positive => there \ is\ minima\ at\ f'(q)=0 \\\\
for \ q= & \ 0 \ or \ q=n-1, \ f(q) \ is \ maximum \ as \ they're \ extreme \ points  \\\\ 
T(n) & \ge c(0^2 +(n - 0 - 1 )^2)  + \Theta(n) \\\\
& \ge c(n - 1 )^2  + \Theta(n) \\\\
& \ge cn^2 \\\\
\end{aligned}
$$
