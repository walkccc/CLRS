...
If the shortest path of A to C contains 2 edges (A->B->C), which is shorter than the direct edge (A->C). Minus w* for reweighting will result in the path with more edges may become longer, leading to an incorrect result.  
![example](img/25.3-4.jpg)
