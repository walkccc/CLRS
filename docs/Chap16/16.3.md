...
Let us consider a set of $n$ symbols $\Sigma=\{ c_i ~|~ 1 \leq i \leq n \}$ such that for each $i$ we have $c_i.freq = a_i$. We shall prove that the Huffman code for this set of symbols given by the run of algorithm HUFFMAN from CLRS is the following code:
- $code(c_n) = 0$
...
- $(\forall i; 3 \leq i \leq n)~ T_i.left = c_i$, $T_i.right = T_{i-1}$ and $T_i.freq = c_i.freq + T_{i-1}.freq$
...
We will prove $p \implies q$ by contradiction:
$$p \implies q \equiv \neg p \vee q \equiv \neg (p \wedge \neg q)$$

Assume that $c_1.freq \ge c_2.freq \ge \cdots \ge c_n.freq \wedge d_T(c_1) > d_T(c_2) > \cdots > d_T(c_n)$

If we take $c_n$ and $c_{n - 1}$, they would have the greatest depth which would imply that:

$$d_T(c_{n - 2}) \leq d_T(c_{n - 1})$$ which would be a contradiction.
