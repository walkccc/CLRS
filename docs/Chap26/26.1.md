...
Solution:
     We know the maximum flow of a graph $G=(V,E)$ with source s and destination t is
$|f|= \sum{f(s,v)}$ with $v \in V$
the value of the maximum flow if effected by the vertexs between s and t.
We know every vertex $v \in V$ must obey the Flow conservation rule, if we can add or delete some vertexs between s and t without changing $|f|$ or violating the Flow conversation rule, and the new graph $G'=(V',E')$ will have the same maximum flow as the original graph $G$
That's why we can replace edge $(u,v)$ by new edges $(u,x)$ and $(x,v)$ with $c(u,x)=c(x,v)=c(u,v)$
After doing so, vertex $v1$ and $v2$ still obeys the Flow conservation rule since the value flows in to or flows out of $v1$ and $v2$ doesn't change at all.
Meanwhile, the value $|f|= \sum{f(s,v)}$ with $v \in V$ still remains the same.
In fact, we can split any edges in this way, even if two vertex $u$ and $v$ doesn't have any connection between them, we can still add a vertex y and make $c(u,y)=0$ 、$c(y,v)=0$.
In this way, we can transform any graph with or without antiparallel edges into an equivalent graph without antiparallel edges and having the same maximum flow value.
