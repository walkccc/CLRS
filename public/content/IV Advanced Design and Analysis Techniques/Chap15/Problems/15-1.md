...
We claim that $p^\prime$ is a longest path from $v$ to $t$.

To prove the claim, we use a cut-and-paste argument. If $p^\prime$ were not a longest path, then there exists a longer path $p^{\prime\prime}$ from $v$ to $t$. We could cut out $p^\prime$ and paste in $p^{\prime\prime}$ to produce a path $u \to v \overset{p^{\prime\prime}}{\leadsto} t$ which is longer than $p$, thus contradicting the assumption that $p$ is a longest path from $u$ to $t$.

It is important to note that the graph is _acyclic_. Because the graph is acyclic, path $p^{\prime\prime}$ cannot include the vertex u, for otherwise there would be a cycle of the form $u \to v \leadsto u$ in the graph. Thus, we can indeed use $p^{\prime\prime}$ to construct a longer path. The acyclicity requirement ensures that by pasting in path $p^{\prime\prime}$ , the overall path is still a _simple_ path (there is no cycle in the path). This difference between the cyclic and the acyclic case allows us to use dynamic programming to solve the acyclic case.
