...
>
> It helps to think of treaps in the following way. Suppose that we insert nodes $x\_1, x\_2, \ldots,x\_n$, with associated keys, into a treap. Then the resulting treap is the tree that would have been formed if the nodes had been inserted into a normal binary search tree in the order given by their (randomly chosen) priorities, i.e., $x\_i.priority < x\_j.priority$ means that we had inserted $x\_i$ before $x\_j$.
>
> **a.** Show that given a set of nodes $x\_1, x\_2, \ldots, x\_n$, with associated keys and priorities, all distinct, the treap associated with these nodes is unique.
>
...
> </div>
>
> **f.** Show that $X\_{ik} = 1$ if and only if $y.priority > x.priority$, $y.key < x.key$, and, for every $z$ such that $y.key < z.key < x.key$, we have $y.priority < z.priority$.
>
> **g.** Show that
>
> <div>
